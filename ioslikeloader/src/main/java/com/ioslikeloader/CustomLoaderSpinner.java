...
import com.customloader.R;

public class CustomLoaderSpinner extends AppCompatImageView {
    ...
    public CustomLoaderSpinner(Context context) {
        super(context);
    ...
    public CustomLoaderSpinner(Context context, AttributeSet attrs) {
        super(context, attrs);
        initCustomAttr(context, attrs);
    }

    public CustomLoaderSpinner(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public CustomLoaderSpinner(Context context, @ColorInt int spinnerColor) {
        this(context, spinnerColor, DEFAULT_DURATION);
    }

    public CustomLoaderSpinner(Context context, @ColorInt int spinnerColor, int duration) {
        this(context, spinnerColor, duration, DEFAULT_CLOCKWISE);
    }

    public CustomLoaderSpinner(Context context, @ColorInt int spinnerColor, int duration, boolean clockwise) {
        super(context);
    ...
    private void initCustomAttr(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.getTheme().obtainStyledAttributes(
                ...
                0, 0);
        try {
            duration = typedArray.getInteger(R.styleable.CustomSpinner_duration, DEFAULT_DURATION);
            spinnerColor = typedArray.getColor(R.styleable.CustomSpinner_spinnerColor, DEFAULT_COLOR);
            clockwise = typedArray.getBoolean(R.styleable.CustomSpinner_clockwise, DEFAULT_CLOCKWISE);
            AnimationDrawable spinnerAnimation = createSpinner(context, spinnerColor, duration, clockwise);
            updateSpinner(spinnerAnimation);
        } finally {
            typedArray.recycle();
